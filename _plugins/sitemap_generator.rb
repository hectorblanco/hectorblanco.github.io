...
module Jekyll
  ...
  MY_URL = "http://www.hectorblanco.eu"
  ...
  # Variable to determine whether the post/page should be excluded or not (default is `false`)
  EXCLUDE_SITEMAP_VARIABLE_NAME = "exclude_sitemap"
  ...
  class Page
    ...
    def location_on_server(my_url)
      ...
      location.gsub(/dev\/dev\//, "dev/").gsub(/index.html$/, "")
    end
  end
  ...
  class SitemapGenerator < Generator
    ...
    def excluded?(page_or_post)
      name = page_or_post.name
      excluded = EXCLUDED_FILES.include? name
      if !excluded
        excluded = !!page_or_post.data[EXCLUDE_SITEMAP_VARIABLE_NAME]
      end
      excluded
    end
